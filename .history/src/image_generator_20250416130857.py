    ...
    def generate_images_for_script(self, script, audio_files_info=None, visual_source="search", visual_timing_mode="sync_to_audio"):
            ...
            if visual_timing_mode == 'overall_theme_fixed_duration':
                ...
                theme_queries = self._generate_theme_queries(
                    script.get('title', ''), # Dùng tiêu đề chính của video
                    query_count,
                    script.get('language', 'en')
                )
                if not theme_queries:
                    logger.error("Failed to generate theme queries. Cannot proceed with theme visuals.")
                    return media_items

                # 4. Tạo/Tìm visuals cho từng theme query (LOGIC MỚI)
                theme_visuals_generated = []
                visual_count = 0
                max_attempts_per_query = 3 # Số lần thử nếu một phương thức (search/ai) thất bại

                # --- CỐ GẮNG LẤY NHIỀU VISUAL HƠN ---
                # Tạo một danh sách các query/prompt để làm việc, có thể lặp lại nếu cần
                queries_to_process = []
                if theme_queries:
                    # Lặp lại danh sách queries gốc đủ số lần để có khả năng đạt target_pool_size
                    # Ví dụ: nếu cần 98 visuals, có 7 queries -> lặp lại mỗi query ~14 lần
                    repeat_factor = math.ceil(target_pool_size / len(theme_queries)) if len(theme_queries) > 0 else 1
                    queries_to_process = (theme_queries * repeat_factor)[:target_pool_size] # Lấy đủ số lượng cần
                    random.shuffle(queries_to_process) # Xáo trộn để tránh lặp lại liên tiếp cùng query
                else:
                    # Fallback nếu không tạo được query: Dùng title làm query duy nhất
                    logger.warning("No theme queries generated. Using main title as fallback query.")
                    queries_to_process = [script.get('title', 'abstract background')] * target_pool_size

                logger.info(f"Expanded query list to {len(queries_to_process)} items to attempt reaching target pool size {target_pool_size}.")

                # Lặp qua danh sách queries đã mở rộng
                for idx, current_query in enumerate(queries_to_process):
                    if visual_count >= target_pool_size: break # Đã đủ, dừng sớm

                    logger.info(f"Processing expanded query {idx + 1}/{len(queries_to_process)}: '{current_query[:80]}...' (Target Pool: {target_pool_size}, Current: {visual_count})")

                    # Cố gắng tạo/tìm visual cho query này
                    visual_path, visual_type = self._find_or_generate_single_visual(
                        query=current_query,
                        visual_source=visual_source,
                        project_media_dir=project_media_dir,
                        # Tạo tên file duy nhất hơn để tránh ghi đè nếu cùng query được lặp lại
                        base_filename=f"theme_expanded_{idx + 1}"
                    )

                    if visual_path:
                        # --- QUAN TRỌNG: Kiểm tra trùng lặp đường dẫn ---
                        # Tránh thêm cùng một file nhiều lần (ví dụ cache trả về cùng ảnh cho query giống nhau)
                        is_duplicate = any(item['path'] == visual_path for item in theme_visuals_generated)
                        if not is_duplicate:
                            theme_visuals_generated.append({
                                ...
                                "query_source": current_query # Vẫn lưu query gốc của lần thử này
                            })
                            visual_count += 1
                            logger.info(f"  Success. Total collected: {visual_count}")
                        else:
                            logger.debug(f"  Skipped duplicate visual: {os.path.basename(visual_path)}")
                            # Không tăng visual_count nếu là duplicate
                    else:
                        logger.warning(f"  Attempt failed for expanded query '{current_query[:80]}...'.")
                        # Không cần sleep ở đây vì đang lặp qua danh sách query mở rộng

                # --- Kết thúc vòng lặp qua queries_to_process ---
                ...
                if visual_count == 0:
                    ...
                    logger.warning(f"Collected only {visual_count} theme visuals, less than estimated required {estimated_visual_slots}. Video might loop last visual.")

                media_items.extend(theme_visuals_generated)
