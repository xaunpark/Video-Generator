        ...
        try:
            ...
            if media_type == 'image':
                ...
                if apply_animation:
                    ...
                    zoompan_base = f"zoompan=d={total_frames}:s={self.width}x{self.height}:fps={self.fps}"

                    if animation_type == "zoom":
                        zoom_expr = f"'1+({intensity}*{ease_expr})'"
                        x_expr = "'(iw-iw/zoom)/2'"
                        y_expr = "'(ih-ih/zoom)/2'"
                        vf_filter_parts.append(f"{zoompan_base}:z={zoom_expr}:x={x_expr}:y={y_expr}")
                        ...
                        vf_filter_parts.append(f"{zoompan_base}:z={zoom_level}:x={x_expr}:y={y_expr}")

                    # --- SỬA LỖI: Thêm trim và format SAU zoompan ---
                    vf_filter_parts.append(f"trim=duration={target_duration:.5f}") # Cắt chính xác theo duration
                    ...
                    vf_filter_parts.append(f"trim=duration={target_duration:.5f}") # Vẫn cần trim cho ảnh tĩnh
                ...
                cmd = [
                    self.ffmpeg_path, "-y",
                    "-loop", "1", "-i", media_path,
                    # Loại bỏ -t str(target_duration) ở đây
                    "-vf", final_vf_filter,
                    "-c:v", "libx264", "-preset", "medium", "-crf", "23",
                    "-r", str(self.fps),
                    # Không cần -frames:v vì trim đã xử lý
                    "-an", output_path
                ]
