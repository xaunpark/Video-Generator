...
class VideoEditor:
    ...
    def _prepare_image_with_effects(self, image_path, output_dir, index):
        """Chuẩn bị hình ảnh với hiệu ứng Ken Burns và Blur Background với chuyển động mượt mà hơn
        ...
        try:
            ...
            # Tăng frame rate để có chuyển động mượt mà hơn
            fps = 60  # Tăng fps để có chuyển động mượt mà hơn
            ...
            easing_expression = f"'if(gte(on,{fps*duration}),{zoom_end},{zoom_start}+(({zoom_end}-{zoom_start})*sin(PI*on/({fps*duration}*2)))'"
            pos_easing_x = f"'iw/2+iw*{x_start}+iw*({x_end}-{x_start})*sin(PI*on/({fps*duration}*2))'"
            pos_easing_y = f"'ih/2+ih*{y_start}+ih*({y_end}-{y_start})*sin(PI*on/({fps*duration}*2))'"
            ...
            command = [
                ...
                f"zoompan=z={easing_expression}:x={pos_easing_x}:y={pos_easing_y}:d={fps*duration}:s={self.width}x{self.height}[ken]; "
                f"[blurred][ken]overlay=(W-w)/2:(H-h)/2:format=auto,minterpolate='mi_mode=mci:mc_mode=aobmc:me_mode=bidir:mb_size=16:vsbmc=1:fps={fps}'",
                "-c:v", "libx264",
                "-pix_fmt", "yuv420p",
                "-r", str(fps),
                "-preset", "slow",  # Sử dụng preset slow để chất lượng tốt hơn
                "-crf", "18",  # Chất lượng cao hơn
            ...
            if self._run_ffmpeg_command(command):
                ...
                # Loại bỏ minterpolate (có thể tiêu tốn nhiều tài nguyên) và đơn giản hóa các bộ lọc
                command_simple = [
                    self.ffmpeg_path,
                    "-y",
                    "-loop", "1",
                    ...
                    f"zoompan=z='1+(0.05*sin(PI*on/({fps*duration}*2)))':d={fps*duration}:s={self.width}x{self.height}",
                    "-c:v", "libx264",
                    "-pix_fmt", "yuv420p",
                    "-r", str(fps),
                    "-preset", "medium",
