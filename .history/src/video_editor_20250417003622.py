...
class VideoEditor:
    ...
    def _create_temp_visual_clip(self, media_item, target_duration, output_path):
        ...
        try:
            ...
            cmd = []
            if media_type == 'image':
                # --- BẮT ĐẦU LOGIC THÊM HIỆU ỨNG ---
                vf_filter_parts = []
                animation_type = VIDEO_SETTINGS.get("image_animation", "none")
                intensity = VIDEO_SETTINGS.get("animation_intensity", 0.03)
                total_frames = max(1, int(round(target_duration * self.fps))) # Tổng số frame

                # Chọn kiểu animation nếu là random
                if animation_type == "random":
                    # Chỉ pan nếu đủ thời gian và intensity đủ lớn để thấy rõ
                    choices = ["zoom"] # mặc định là zoom
                    if target_duration > 1.0 and intensity > 0.01:
                         choices.extend(["pan_left", "pan_right"]) # nếu đủ thời gian thì mới pan
                    animation_type = random.choice(choices)

                apply_animation = animation_type != "none" and target_duration > 0.5 # Chỉ áp dụng nếu đủ dài và khác "none"

                if apply_animation:
                    logger.debug(f"  Applying animation '{animation_type}' (intensity: {intensity})")

                    # Biểu thức easing (cosine ease-in-out) cho chuyển động mượt mà
                    # p = on / total_frames (tiến trình từ 0 đến 1)
                    # Thêm 'max(0,on-1)' để tránh lỗi log ở frame đầu (on=0)
                    # Sử dụng 'ld(1)' để lấy total_frames thay vì truyền trực tiếp vào string (an toàn hơn)
                    ease_expr = f'(0.5*(1-cos(PI*max(0,on-1)/ld(1))))'

                    if animation_type == "zoom":
                        zoom_expr = f"'1+({intensity}*{ease_expr})'"
                        x_expr = "'(iw-iw/zoom)/2'" # Giữ tâm zoom ở giữa
                        y_expr = "'(ih-ih/zoom)/2'"
                        vf_filter_parts.append(f"zoompan=z={zoom_expr}:x={x_expr}:y={y_expr}:d=ld(1):s={self.width}x{self.height}:fps={self.fps}")

                    elif animation_type in ["pan_left", "pan_right"]:
                        zoom_level = f"'1+{intensity*0.1}'" # Zoom nhẹ khi pan để tránh viền đen
                        max_x_offset = f"(iw*{zoom_level}-{self.width})" # Offset tối đa theo chiều ngang sau khi zoom

                        if animation_type == "pan_left": # Pan từ phải sang trái (hiển thị phần bên trái)
                            x_expr = f"'{max_x_offset}*(1-{ease_expr})'"
                        else: # pan_right - Pan từ trái sang phải (hiển thị phần bên phải)
                            x_expr = f"'{max_x_offset}*{ease_expr}'"

                        y_expr = f"'(ih*{zoom_level}-{self.height})/2'" # Giữ tâm y ở giữa
                        vf_filter_parts.append(f"zoompan=z={zoom_level}:x={x_expr}:y={y_expr}:d=ld(1):s={self.width}x{self.height}:fps={self.fps}")

                    # Luôn thêm format pix_fmt vào cuối filter zoompan
                ...
                cmd = [
                    self.ffmpeg_path, "-y",
                    "-loop", "1", "-i", media_path,
                    # --- QUAN TRỌNG: Truyền total_frames vào bằng -eval_mode flag ---
                    # Điều này cho phép dùng ld(1) trong biểu thức filter
                    "-eval_mode", "init", "-frames:v", str(total_frames),
                    # --------------------------------------------------------------
                    "-vf", final_vf_filter,
                    "-c:v", "libx264", "-preset", "medium", "-crf", "23", # Có thể thử 'fast' nếu 'medium' quá chậm
