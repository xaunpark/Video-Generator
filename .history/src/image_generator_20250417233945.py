...
class ImageGenerator:
    ...
    def generate_images_for_script(self, script, audio_files_info=None, visual_source="search", visual_timing_mode="sync_to_audio"):
            ...
            # --- KHỞI TẠO VIDEO FINDER (CHỈ KHI CẦN) ---
            self.video_finder = None
            if visual_source in ["search", "video_only"] and VIDEO_SETTINGS.get("enable_video_clips", False):
                try:
                    self.video_finder = VideoClipFinder()
                    logger.info("VideoClipFinder initialized.")
                except ImportError as ie:
                    logger.error(f"Cannot import VideoClipFinder: {ie}. Disabling video clips for this run.")
                    VIDEO_SETTINGS["enable_video_clips"] = False # Tắt tạm thời cho lần chạy này
                except Exception as vf_err:
                    logger.error(f"Error initializing VideoClipFinder: {vf_err}. Disabling video clips.")
                    self.video_finder = None
            ...
            self.fallback_video_files = glob.glob(os.path.join(self.fallback_video_dir, "*.mp4"))
            ...
            # --- CHẾ ĐỘ SYNC TO AUDIO (LOGIC CŨ) ---
                ...
                for i, scene in enumerate(script.get('scenes', [])):
                    ...
                    temp_video_fallback_path = os.path.join(project_media_dir, f"{scene_base_filename}_vid_fallback.mp4") # Cho fallback video
