        ...
        try:
            ...
            if media_type == 'image':
                ...
                if apply_animation:
                    ...
                    total_frames = max(1, int(round(target_duration * self.fps))) # Tổng số frame cho clip
                    ...
                    logger.debug(f"  Applying random animation: {animation_type} (intensity: {intensity}, easing: COSINE)")

                    # --- ÁP DỤNG COSINE EASING ---
                    # Biểu thức easing dùng chung (p = on / total_frames)
                    # Lưu ý: Cần đảm bảo total_frames > 0, đã làm ở trên
                    # Thêm 'max(0,on-1)' để tránh lỗi log khi on=0 ở frame đầu
                    ease_expr = f'(0.5*(1-cos(PI*max(0,on-1)/{total_frames})))'

                    if animation_type == "zoom":
                        # Zoom từ 1 đến 1 + intensity với easing
                        ...
                        # Giữ zoom cố định nhẹ
                        zoom_level = f"'1+{intensity*0.5}'" # Có thể giảm nhẹ intensity cho pan
                        max_x_offset = f"(iw*{zoom_level}-{self.width})" # Tính offset tối đa

                        if animation_type == "pan_left": # Đi từ max về 0
                            x_expr = f"'{max_x_offset}*(1-{ease_expr})'"
                        else: # pan_right - Đi từ 0 đến max
                            x_expr = f"'{max_x_offset}*{ease_expr}'"

                        # Giữ y ở giữa (cần tính toán dựa trên zoom_level)
                    ...
                    # --- KẾT THÚC ÁP DỤNG COSINE EASING ---
                ...
                cmd = [
                    ...
                    "-vf", final_vf_filter,
                    # --- QUAN TRỌNG: Thử preset nhanh hơn KHI dùng easing ---
                    "-c:v", "libx264", "-preset", "fast", "-crf", "23", # Start with 'fast', can adjust later to 'medium' or 'ultrafast' or 'veryslow' or 'ultra' if needed 
                    # ------------------------------------------------------
