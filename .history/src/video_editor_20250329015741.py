    ...
    def process_scene_media(self, media_item, audio_path, output_path):
        ...
        if media_type == 'image':
            ...
            try:
                ...
                # Đảm bảo kích thước ảnh đúng trước khi áp dụng hiệu ứng
                ...
                # Thêm hiệu ứng chuyển động đơn giản nếu được bật
                if self.enable_ken_burns:  # Giữ tên setting giống cũ để tương thích
                    logger.info(f"Áp dụng hiệu ứng zoom nhẹ nhàng theo hàm sin")
                    
                    try:
                        # Định nghĩa hiệu ứng zoom nhẹ theo hàm sin
                        # Zoom giữa 100% và 103% một cách mượt mà
                        zoom_amount = 0.03  # Zoom tối đa 3%
                        
                        # Sử dụng hàm sin để tạo hiệu ứng mượt mà
                        # sin dao động từ -1 đến 1, ta biến đổi thành dao động từ 1.0 đến 1.0+zoom_amount
                        def simple_zoom(t):
                            # Hoàn thành một chu kỳ sin trong suốt thời lượng clip
                            progress = t / audio_duration * math.pi * 2  # 0 đến 2π
                            # Biến đổi sin (-1 đến 1) thành hệ số zoom (1.0 đến 1.0+zoom_amount)
                            return 1.0 + (zoom_amount/2) * (math.sin(progress) + 1)
                        
                        # Áp dụng hiệu ứng
                        image_clip = image_clip.resize(simple_zoom)
                    except Exception as e:
                        logger.warning(f"Lỗi khi áp dụng hiệu ứng zoom: {str(e)}. Sử dụng ảnh tĩnh.")
                        # Trở lại ảnh tĩnh nếu hiệu ứng bị lỗi
                        image_clip = ImageClip(media_path, duration=audio_duration)
                        if image_clip.size != (self.width, self.height):
                            image_clip = image_clip.resize((self.width, self.height))
