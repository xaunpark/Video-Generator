...
class VideoEditor:
    ...
    def _create_temp_visual_clip(self, media_item, target_duration, output_path):
        ...
        logger.debug(f"Creating temp visual clip (Simplified Logic) for Scene/Item '{scene_num}' ({media_type}, Target: {target_duration:.2f}s) -> {os.path.basename(output_path)}")
        ...
        try:
            ...
            if media_type == 'image':
                vf_filter_parts = []
                animation_type_setting = VIDEO_SETTINGS.get("image_animation", "none") # Đọc setting gốc
                intensity = VIDEO_SETTINGS.get("animation_intensity", 0.03)
                total_frames = max(1, int(round(target_duration * self.fps)))

                # === ÁP DỤNG LOGIC ZOOM ĐƠN GIẢN TỪ CODE CŨ ===
                # Chỉ áp dụng zoom nếu setting là "zoom" hoặc "random" (để test) và đủ dài
                apply_linear_zoom = (animation_type_setting in ["zoom", "random"]) and target_duration > 0.5

                if apply_linear_zoom:
                    logger.debug(f"  Applying SIMPLIFIED linear zoom (intensity: {intensity})")
                    # Biểu thức zoom tuyến tính
                    zoom_expr = f"'1+({intensity}*on/{total_frames})'"
                    x_expr = "'(iw-iw/zoom)/2'" # Giữ tâm
                    y_expr = "'(ih-ih/zoom)/2'" # Giữ tâm
                ...
                # === KẾT THÚC ÁP DỤNG LOGIC CŨ ===

                # --- VẪN GIỮ FILTER TRIM VÀ FORMAT ---
                vf_filter_parts.append(f"trim=duration={target_duration:.5f}") # Cắt chính xác
