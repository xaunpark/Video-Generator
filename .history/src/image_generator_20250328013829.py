...
class ImageGenerator:
    def __init__(self):
        ...
        if not self.openai_api_key:
            # Warn if OpenAI key is missing, but allow running with fallback
    ...
    def _call_openai_for_keywords(self, prompt):
        """Calls the OpenAI API to get image keywords.

        Args:
            prompt (str): The prompt to send to OpenAI.

        Returns:
            str: Suggested keywords from OpenAI, or None on error.
        """
        if not self.openai_api_key:
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": "You are an expert at extracting the most visually descriptive keywords from a text snippet for image searches. Output only 3-5 relevant keywords in English, separated by spaces. Do not include explanations or introductory phrases like 'Keywords:'."},
                    {"role": "user", "content": prompt}
                ],
                "temperature": 0.5, # Lower temperature for more consistent results
                "max_tokens": 25    # Limit tokens for concise keywords
            }

            logger.debug(f"Calling OpenAI for keywords with prompt: {prompt[:100]}...")
            response = requests.post(url, headers=self.openai_headers, json=payload, timeout=20) # Added timeout

            if response.status_code == 200:
                data = response.json()
                if 'choices' in data and data['choices']:
                    keywords = data['choices'][0]['message']['content'].strip()
                    # Clean up the result: remove quotes, colons, hash symbols, trailing periods
                    keywords = keywords.replace('"', '').replace("'", '').replace(':', '').replace('#', '').rstrip('.').strip()
                    # Remove potential introductory phrases if the AI still includes them
                    ...
                    if len(keywords.split()) > 7:
                        logger.warning(f"OpenAI keywords too long after cleaning, taking first 5: '{keywords}'")
                        keywords = ' '.join(keywords.split()[:5])
                    # Check if keywords are empty after cleaning
                    if not keywords:
                         logger.warning("OpenAI returned empty keywords after cleaning.")
                         return None
                    return keywords
                else:
            ...
            logger.error(f"Error calling OpenAI API for keywords: {str(e)}", exc_info=True) # Log full traceback
            return None

    def _extract_keywords_basic(self, text, max_words=10):
        """Extracts basic keywords from text (fallback method)."""
        if not text or not text.strip():
            return "news image" # Default for empty text

        text = text.strip()
        words = text.split()

        if len(words) <= max_words:
            return text.rstrip('.').strip() # Remove trailing period if present

        # Get the first complete sentence
        sentences = text.split('.')
        # Filter out empty strings that might result from leading/multiple periods
        valid_sentences = [s.strip() for s in sentences if s.strip()]
        first_sentence = valid_sentences[0] if valid_sentences else text # Use full text if no valid sentence found
    ...
    def _extract_keywords(self, text):
        """Extracts image search keywords from scene content, preferring OpenAI.

        Args:
            text (str): The content of the scene.

        Returns:
            str: Keywords for image search.
        """
        if not text or not text.strip():
        ...
        prompt = f"""
        Extract the best 3-5 visually descriptive English keywords for searching images related to the following news scene content.
        Focus on the main subject, action, and specific locations or entities if mentioned. Be concise.
        Output only the keywords in English, separated by spaces.
        ...
        if openai_keywords:
            # Validate the result (not too long, doesn't contain instructions, etc.)
        ...
        # Fallback to basic method if OpenAI fails or returns invalid results
        logger.info("Falling back to basic keyword extraction method.")
        basic_keywords = self._extract_keywords_basic(text_to_process) # Use default max_words (10)
    ...
    def generate_images_for_script(self, script):
        ...
        for scene in script.get('scenes', []):
            ...
            try:
                ...
                # --- Step 3.1: Extract Keywords ---
                keywords = self._extract_keywords(scene_content) # Uses OpenAI with fallback
                ...
                try:
                    ...
                    try:
                        # Stage 2: Try simpler search (always using basic keyword extraction)
                        logger.info(f"Stage 2: Trying simplified search")
                        simplified_keywords = self._extract_keywords_basic(scene_content, max_words=5) # Get first 5 words
                        simplified_query = f"{simplified_keywords} news photo" # Simple query
    ...
    def _search_and_download_image(self, query, output_path):
        ...
        try:
            ...
            for img_data in image_results:
                ...
                # Filter out very small images or those without URLs
                if not url or width < 400 or height < 300:
                    continue
            ...
            if not potential_images:
                 ...
                 potential_images = [{"url": img.get("imageUrl"), "score": 0, "width": img.get("width",0), "height": img.get("height",0)}
    ...
    def _resize_image(self, image):
        ...
        try:
            ...
            if abs(img_ratio - target_ratio) < 0.01:
                logger.debug(f"Resizing image ({image.width}x{image.height}) to target ({self.width}x{self.height})")
                return image.resize((self.width, self.height), Image.Resampling.LANCZOS)

            logger.debug(f"Cropping and resizing image ({image.width}x{image.height}) to fit target ratio {target_ratio:.2f}")
    ...
    def _create_search_query(self, keywords, title):
        """Creates an optimized search query for the Serper API (English)."""
        # Clean and limit input keywords
        keywords = keywords.replace('"', '').replace("'", '').replace(':', '').replace('#', '').strip()
        keywords = ' '.join(keywords.split()[:7]) # Limit to 7 words

        # Extract a few relevant keywords from the title (if provided)
        title_keywords = ""
        if title:
             title_words = title.split()[:4] # Take first 4 words
             # Filter out common English stop words
             common_words = {"the", "a", "an", "is", "are", "in", "on", "at", "to", "for", "of", "and", "with", "as", "it"}
             title_keywords = ' '.join(w for w in title_words if w.lower() not in common_words and len(w) > 2)

        # Combine keywords, prioritizing the extracted ones
        if len(keywords.split()) >= 3: # If extracted keywords are reasonably long
             query = f"{keywords} {title_keywords}"
        elif title_keywords: # If keywords are short but title keywords exist
             query = f"{title_keywords} {keywords}"
        else: # If both are short or empty
             query = keywords if keywords else "news footage" # Final fallback query

        # Add a relevant suffix for image search
        # suffix = random.choice(["photo", "image", "illustration", "news photo", "news image"])
        suffix = "news photo hd" # Consistent suffix
        final_query = f"{query.strip()} {suffix}".strip()

        # Ensure the final query is not excessively long (Serper might have limits)
