...
class ImageGenerator:
    ...
    def _check_and_download_fonts(self):
        ...
        # Nếu font chưa tồn tại, tải về
        if not os.path.exists(font_path):
            try:
                logger.info("Đang tải font Roboto-Bold.ttf...")
                ...
                logger.warning("Sẽ sử dụng font mặc định")
    
    def _get_font(self, size=40):
        """Lấy font với kích thước cụ thể"""
        try:
            # Thử sử dụng TrueType font
            font_path = os.path.join(self.fonts_dir, "Roboto-Bold.ttf")
            return ImageFont.truetype(font_path, size)
        except Exception as e:
            logger.warning(f"Không thể tải font TrueType: {str(e)}")
            try:
                # Thử sử dụng font mặc định với kích thước
                return ImageFont.load_default().font_variant(size=size)
            except:
                # Nếu không thể đặt kích thước, sử dụng font mặc định
                logger.warning("Không thể đặt kích thước font mặc định, sử dụng kích thước mặc định")
