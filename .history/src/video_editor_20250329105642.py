...
class VideoEditor:
    ...
    def process_scene_media(self, media_item, audio_path, output_path):
        ...
        if media_type == 'image':
            # Xử lý ảnh tĩnh (không có hiệu ứng)
            try:
                # Tạo video tạm từ ảnh (không có audio)
                temp_video = output_path + ".temp.mp4"
                
                # Sử dụng ffmpeg với cấu hình cực kỳ ổn định
                image_cmd = [
                    "ffmpeg", "-y",
                    "-loop", "1",
                    "-i", media_path,
                ...
                logger.info(f"Tạo video từ ảnh tĩnh: {' '.join(image_cmd)}")
                ...
                logger.info(f"Đã xử lý ảnh tĩnh thành công: {output_path}")
