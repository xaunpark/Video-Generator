...
class ImageGenerator:
    def __init__(self):
        ...
        # --- THÊM KHỞI TẠO FALLBACK VIDEO ---
        self.fallback_video_dir = os.path.join(self.assets_dir, "fallback_videos")
        os.makedirs(self.fallback_video_dir, exist_ok=True)
        # Lấy danh sách các file video hỗ trợ (ví dụ: mp4, mov, avi)
        supported_video_extensions = ("*.mp4", "*.mov", "*.avi", "*.webm")
        self.fallback_video_files = []
        for ext in supported_video_extensions:
            self.fallback_video_files.extend(
                glob.glob(os.path.join(self.fallback_video_dir, ext))
            )

        if not self.fallback_video_files:
            logger.warning(f"Fallback video directory is empty or contains no supported videos: {self.fallback_video_dir}")
            logger.warning("Please add .mp4, .mov, .avi, or .webm files to use video fallback.")
        else:
             logger.info(f"Found {len(self.fallback_video_files)} fallback videos in {self.fallback_video_dir}")
...
def _use_local_fallback_video(self, query, output_path):
    """
    Selects a random fallback video from assets/fallback_videos and copies it.

    Args:
        query (str): The original search query (currently unused for random selection).
        output_path (str): The destination path to copy the fallback video to.

    Returns:
        str: The output_path if successful, None otherwise.
    """
    if not self.fallback_video_files:
        logger.error("Cannot use local fallback video: No fallback videos found in assets/fallback_videos.")
        return None

    try:
        # Chọn ngẫu nhiên một video từ danh sách đã được load trong __init__
        selected_fallback_video = random.choice(self.fallback_video_files)
        logger.info(f"Using local fallback video: {os.path.basename(selected_fallback_video)} for output: {os.path.basename(output_path)}")

        # Copy video đã chọn vào đường dẫn output yêu cầu
        shutil.copy2(selected_fallback_video, output_path) # copy2 preserves metadata

        if os.path.exists(output_path) and os.path.getsize(output_path) > 1000: # Kiểm tra file hợp lệ
            return output_path
        else:
            logger.error(f"Failed to copy or validate fallback video to: {output_path}")
            return None
    except IndexError: # Trường hợp self.fallback_video_files rỗng dù đã check
         logger.error("Fallback video list became empty unexpectedly.")
         return None
    except Exception as e:
        logger.error(f"Error using local fallback video: {e}", exc_info=True)
        # Xóa file output nếu copy bị lỗi
