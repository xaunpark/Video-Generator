...
class VideoEditor:
    ...
    def process_scene_media(self, media_item, audio_path, output_path):
        ...
        if media_type == 'image':
            ...
            try:
                # Read image and create clip with duration equal to audio
                image_clip = ImageClip(media_path, duration=audio_duration)
                
                # Ensure image is correctly sized first
                if image_clip.size != (self.width, self.height):
                    image_clip = image_clip.resize((self.width, self.height))
                
                # Simple subtle animation instead of complex Ken Burns
                if self.enable_ken_burns:  # Keep the same setting name for compatibility
                    logger.info(f"Applying simple gentle zoom animation")
                    
                    # Define a subtle sine-based zoom effect
                    # This will gently zoom between 100% and 103% size
                    zoom_amount = 0.03  # Maximum 3% zoom
                    
                    # Use a simple sine function for smooth animation
                    # sin oscillates between -1 and 1, we transform it to oscillate between 1.0 and 1.0+zoom_amount
                    def simple_zoom(t):
                        # Complete one full sine wave during the clip duration
                        progress = t / audio_duration * 3.14159 * 2  # 0 to 2π
                        # Transform sine (-1 to 1) to zoom factor (1.0 to 1.0+zoom_amount)
                        return 1.0 + (zoom_amount/2) * (math.sin(progress) + 1)
                        
                    # Apply the effect - this is much simpler than the Ken Burns effect
                    image_clip = image_clip.resize(simple_zoom)
