    ...
    def process_scene_media(self, media_item, audio_path, output_path):
        ...
        if media_type == 'image':
            ...
            try:
                ...
                if self.enable_ken_burns and not VIDEO_SETTINGS.get("debug_disable_effects", False):
                    # Choose random Ken Burns effect type with reduced intensity
                    ken_burns_type = random.choice(['zoom_in', 'zoom_out', 'pan_left', 'pan_right'])
                    logger.info(f"Applying Ken Burns effect: {ken_burns_type}")
                    
                    # Use smaller zoom/pan factors to reduce strain
                    zoom_amount = 0.03  # Reduced from 0.05
                    
                    # Precompute some values to avoid recalculation for each frame
                    img_width, img_height = image_clip.size
                    
                    if ken_burns_type == 'zoom_in':
                        # Simpler implementation for zoom-in
                        start_size = 1.0
                        end_size = 1.0 + zoom_amount
                        
                        # Create clips at start and end size, then crossfade between them
                        start_clip = image_clip.resize(start_size).set_duration(audio_duration)
                        end_clip = image_clip.resize(end_size).set_duration(audio_duration)
                        
                        # Use composite instead of complex resize function
                        image_clip = CompositeVideoClip([
                            start_clip.crossfadeout(audio_duration),
                            end_clip.crossfadein(audio_duration)
                        ]).set_duration(audio_duration)
                        
                    elif ken_burns_type == 'zoom_out':
                        # Simpler implementation for zoom-out
                        start_size = 1.0 + zoom_amount
                        end_size = 1.0
                    ...
                    elif ken_burns_type == 'pan_left' or ken_burns_type == 'pan_right':
                        # For panning, use a simpler approach
                        # First resize image slightly larger
                        enlarged_clip = image_clip.resize(1.0 + zoom_amount)
                        
                        # Determine the max offset (pixels to move)
                        max_offset_x = int((enlarged_clip.w - self.width) / 2)
                        
                        if max_offset_x <= 0:
                            # Not enough room to pan, fallback to simple zoom
                            image_clip = enlarged_clip.resize(width=self.width).set_duration(audio_duration)
                        else:
                            # Create start and end positions
                            if ken_burns_type == 'pan_left':
                                start_pos = (self.width - max_offset_x, 'center')
                                end_pos = (max_offset_x, 'center')
                            else:  # pan_right
                                start_pos = (max_offset_x, 'center')
                                end_pos = (self.width - max_offset_x, 'center')
                            
                            # Create start and end clips
                            start_clip = enlarged_clip.set_position(start_pos).set_duration(audio_duration)
                            end_clip = enlarged_clip.set_position(end_pos).set_duration(audio_duration)
