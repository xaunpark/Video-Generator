...
class ImageGenerator:
    ...
    def generate_images_for_script(self, script, audio_files_info=None, visual_source="search", visual_timing_mode="sync_to_audio", style_strategy: BaseVideoStyle = None):
            ...
            if visual_timing_mode == 'overall_theme_fixed_duration':
                ...
                final_theme_queries = [] # Khởi tạo rỗng
                query_source_info = "Not Applicable (Local Fallback Video Only)" # Default cho local

                # CHỈ TẠO QUERY NẾU KHÔNG PHẢI LOCAL FALLBACK
                if visual_source != "local_fallback_video_only":
                    ...
                    final_theme_queries = list(theme_queries)
                    query_source_info = "Generated by LLM" # Hoặc override nếu có

                # --- Logic Override Query ---
                ...
                # --- Vòng lặp xử lý từng Query hoặc Slot ---
                # Chúng ta không cần query thực sự, chỉ cần lặp đủ số slot
                # Lấy query gốc từ theme (chủ yếu để log hoặc làm base_filename)
                generated_theme_queries = list(final_theme_queries) # Lấy list query đã tạo (hoặc override)
                from itertools import cycle
                query_cycler = cycle(generated_theme_queries if generated_theme_queries else ["fallback_query"])

                logger.info(f"Starting visual collection loop for {estimated_visual_slots} slots (Mode: {visual_source})...")

                for idx in range(estimated_visual_slots): # Lặp đủ số slot cần
                     # --- KIỂM TRA DỪNG SỚM NGAY ĐẦU VÒNG LẶP ---
                    if len(unique_visuals_collected) >= estimated_visual_slots:
                        logger.info(f"Collected enough unique visuals ({len(unique_visuals_collected)} >= {estimated_visual_slots}). Stopping loop.")
                        break # Thoát khỏi vòng lặp for
                    
                    current_query = next(query_cycler)

                    current_theme_query_for_log = next(query_cycler) # Lấy query để log/làm tên file
                    logger.info(f"--- Processing Visual Slot {idx + 1}/{estimated_visual_slots} (Source: {visual_source}, Base Query: '{current_query[:80]}...') ---")

                    visual_path = None
                    visual_type = "unknown"
                    # Dùng idx để tạo tên file duy nhất
                    temp_base_filename = f"theme_slot_{idx + 1}"

                    # =====================================================================
                    # === LOGIC IF/ELIF/ELSE DỰA TRÊN visual_source (GỌI HÀM HELPER) ===
                    # =====================================================================

                    # --- Option 0.5: visual_source == LOCAL FALLBACK VIDEO ONLY ---
                    if visual_source == "local_fallback_video_only": # << ĐÂY LÀ CHECK ĐẦU TIÊN
                        logger.info(f"Slot {idx+1}: Source forced to 'local_fallback_video_only'.")
                        ...
                        if visual_path is None:
                            logger.error(f"Slot {idx+1}: FAILED to get local fallback video.")

                        elif visual_type != "video":
                             logger.error(f"  Slot {idx+1}: Expected local fallback video, but got type '{visual_type}'. Treating as failure.")
                             visual_path = None # Đánh dấu thất bại
                             visual_type = "unknown"
                    ...
                    if visual_path and visual_type != "unknown":
                        # QUAN TRỌNG: Đảm bảo chỉ thêm nếu là video khi visual_source là local_fallback_video_only
                        if visual_source == "local_fallback_video_only" and visual_type != "video":
                             logger.error(f"  Slot {idx+1}: Expected local fallback video, but got type '{visual_type}'. Skipping add.")
                             continue
            ...
            ##################################
                ...
                if total_scenes == 0:
                    ...
                    for i, scene in enumerate(scenes_in_script):
                        ...
                        # --- KIỂM TRA SỰ KHÔNG TƯƠNG THÍCH TRƯỚC ---
                        effective_visual_source = visual_source # Bắt đầu với nguồn gốc
                        if visual_source == "local_fallback_video_only":
                            # Log cảnh báo và đổi nguồn hiệu lực sang "search"
                            logger.warning(f"Scene {scene_number}: Visual source 'local_fallback_video_only' is incompatible with 'sync_to_audio' timing mode.")
                            logger.warning(f"--> Falling back to standard 'search' behavior (online video/image) for this scene.")
                            effective_visual_source = "search" # << Ghi đè nguồn chỉ cho lần xử lý này
                        # --- KẾT THÚC KIỂM TRA ---

                        # Log nguồn visual *hiệu lực* sẽ được dùng
                        logger.info(f"--- Processing Scene {scene_number}/{total_scenes} (EffectiveVSrc: {effective_visual_source}, Timing: sync_to_audio, Query: '{search_query}') ---")
                        ...
                        if effective_visual_source == "search":
                        ...
                        elif effective_visual_source == "ai":
                        ...
                        elif effective_visual_source == "video_only":
